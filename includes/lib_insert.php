...
function insert_query_info()
{
    ...
    $online_count = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('sessions'))+50+rand(1,5);

    /* 加入触发cron代码 */
	/*
    $cron_method = empty($GLOBALS['_CFG']['cron_method']) ? '<img src="api/cron.php?t=' . gmtime() . '" alt="" style="width:0px;height:0px;" />' : '';*/

    return sprintf($GLOBALS['_LANG']['query_info'], $online_count, $query_time, $online_count) . $gzip_enabled . $memory_usage . $cron_method;
}
