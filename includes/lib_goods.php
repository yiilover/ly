 ...
 * @return  array
 */
function get_categories_tree($cat_id = 0,$curr_id = 0)
{
    if ($cat_id > 0)
    {
    ...
    if ($GLOBALS['db']->getOne($sql) || $parent_id == 0)
    {
        ...
        foreach ($res AS $row)
        {
            if ($row['is_show'] && $row['cat_name']!='免费模板' )
            {
				...
				$cat_arr[$row['cat_id']]['style'] = $row['cat_id']==$curr_id?'color:#FF0000':'';
                if (isset($row['cat_id']) != NULL)
                {
                    $cat_arr[$row['cat_id']]['cat_id'] = get_child_tree($row['cat_id'],$curr_id);
                }
            }
        }
    }
...
function get_child_tree($tree_id = 0,$id=1)
{
    $three_arr = array();
    ...
    if ($GLOBALS['db']->getOne($sql) || $tree_id == 0)
    {
        ...
        foreach ($res AS $row)
        {
            if ($row['is_show'] )
			   ...
			   $three_arr[$row['cat_id']]['style'] = $row['cat_id']==$id?'color:#FF0000':'';
               if (isset($row['cat_id']) != NULL)
                   {
    ...
    if (empty($type_goods[$type]))
    {
        ...
        if ($data === false)
        {
            $sql = 'SELECT g.goods_id, g.is_best, g.is_new, g.is_hot, g.is_promote, b.brand_name,g.sort_order ' .
               ...
               ' ORDER BY g.sort_order, g.last_update ASC';
        ...
        $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name2, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
        ...
        foreach ($result AS $idx => $row)
        {
            ...
            $goods[$idx]['name2']        = $row['goods_name2'];
...
$row['click_count'] = $row['click_count']+654;
        return $row;
    }
    else
    {
